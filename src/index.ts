...
const APP_URL = 'https://dashing-kringle-61a63d.netlify.app'
...
app.commandLine.appendSwitch(
  ...
  'dashing-kringle-61a63d.netlify.app'
)
...
if (!amMainInstance) {
  ...
  logStream.write(
    `--- Launching HTTP Toolkit desktop v${DESKTOP_VERSION} ---\n`
  )
  ...
  const args = yargs
    .option('with-forwarding', {
      ...
      hidden: true,
      description:
        'Preconfigure a forwarding address, for integration with other tools.',
    })
  ...
  app.on('web-contents-created', (_event, contents) => {
    ...
    contents.on('dom-ready', () => {
      ...
      if (args['with-forwarding'])
        injectValue('httpToolkitForwardingDefault', args['with-forwarding'])
    })
  ...
  function checkForUnsafeNavigation(url: URL) {
    if (url.protocol !== 'http:' && url.protocol !== 'https:') {
      ...
      const error = new Error(
        `Attempt to open a dangerous non-HTTP url: ${url}`
      )
      throw error
    }
  }
  ...
  function handleExternalNavigation(url: URL) {
    shell.openExternal(url.toString()).catch((error) => {
      showErrorAlert(
        'Failed to open URL',
        `HTTP Toolkit could not open ${url.toString()} in your browser, because: ${
  ...
  async function cleanupOldServers() {
    ...
    const serverUpdatesPath =
      process.env.OCLIF_CLIENT_HOME || path.join(oclifDataPath, 'client')

    const serverPaths = await fs
      .readdir(serverUpdatesPath)
  ...
  async function startServer(retries = 2) {
    ...
    const serverShutdown: Promise<void> = new Promise<Error | number | null>(
      (resolve) => {
      ...
      if (errorOrCode && typeof errorOrCode !== 'number') {
        ...
        error = new Error(
          `Server shutdown unexpectedly with code ${errorOrCode}`
        )
      }

      addBreadcrumb({
        category: 'server-exit',
    ...
    .then((proxyConfig) => {
      ...
      if (proxyConfig) {
        ...
        if (
          ...
          proxyHostname.startsWith('127.0.0')
        ) {
      ...
      if (shouldDisableProxy) {
        ...
        ;['http_proxy', 'HTTP_PROXY', 'https_proxy', 'HTTPS_PROXY'].forEach(
          (v) => delete process.env[v]
        )
  ...
  Promise.all([cleanupOldServers().catch(console.log), portCheck])
    .then(() => startServer())
    .catch((err) => {
...
ipcMain.handle(
  'open-context-menu',
  (_event: {}, options: ContextMenuDefinition) => openContextMenu(options)
)
