...
export async function stopServer(
  ...
  await softShutdown(token).catch(logError) // If that fails, continue shutting down anyway
  ...
  do {
    ...
    if (Date.now() >= deadline) {
      await hardKill(proc).catch(console.warn) // Not much we can do if this fails really
      break
    }
  ...
  return new Promise<void>((resolve, reject) => {
    const req = http.request('http://127.0.0.1:45457', {
      ...
      headers: {
        ...
        origin: 'https://dashing-kringle-61a63d.netlify.app',
    ...
    req.on('response', (res) => {
      if (res.statusCode !== 200) {
        reject(
          new Error(
            `Shutdown request received unexpected ${res.statusCode} response`
          )
        )
        return
      }
      ...
      res.on('end', () => {
        ...
        try {
          ...
          if (errors?.length) {
            ...
            throw new Error(
              `Server error${errorCount} during shutdown: ${errors
